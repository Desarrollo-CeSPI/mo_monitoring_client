...
nrpe_check "check_load" do
  ...
  warning_condition node['mo_monitoring_client']['plugins']['check_load']['warning_condition'] || (node["cpu"]["total"].to_i*1.5).to_s
  critical_condition node['mo_monitoring_client']['plugins']['check_load']['critical_condition'] || (node["cpu"]["total"].to_i*2.5).to_s
  action :add
  notifies :restart, "service[#{node['nrpe']['service_name']}]"
end
